...
class Agent extends ApiController
{
    ...
    // $area = $this->input->post('area') ? $this->input->post('area') : $this->input->get('area');
		...
		$agent =  $this->gcore->agent($area, $branch, $unit, $produk, $dateStart, $dateEnd, $page);
		// $data = $agent->data;
		// $total_page = $agent->total_page;
		// var_dump($agent);exit;
    ...
    // $area = $this->input->post('area') ? $this->input->post('area') : $this->input->get('area');
		...
		$data =  $this->gcore->agent_export($area, $branch, $unit, $produk, $dateStart, $dateEnd,$page);
		echo json_encode(array(
			'data'	=> $data,
			'message'	=> 'Successfully Get Data Regular Pawns'
		));
