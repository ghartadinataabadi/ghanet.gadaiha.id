...
class Hps extends ApiController
{
    ...
    $area = $this->input->post('area') ? $this->input->post('area') : $this->input->get('area');
		...
		$data =  $this->syariah->hps($area, $merk, $search, $insurance, $processor)->data;
	...
	public function insert()
	{
		...
		$harga =  preg_replace("/[^0-9]/","",$data);
            ...
            $db = false;
            $db = $this->syariah->insertHps($data);
            if($db==201){
                echo json_encode(array(
                    'data'	=> 	true,
                    'status'=>true,
	...
	public function upload()
	{
		...
		if($this->syariah->uploadHps($data, $region, $insurance_item_id)){
	...
	{
		echo json_encode(array(
			'data'  	=> 	$this->syariah->show($this->input->get("id")),
			'status'	=> true,
	...
	public function update()
	{
      ...
      $data = $this->input->post('edit_harga');
            ...
            $db = false;
            $db = $this->syariah->update($id,$data);
            if($db=200){
                echo json_encode(array(
                    'data'	=> 	 $data,
                    'status'=>true,
	...
	public function merk_hps($id){
		$data = $this->syariah->merk_hps($id)->data;
		echo json_encode(array(
				'data'	=> 	$data,
				'status'=>true,
