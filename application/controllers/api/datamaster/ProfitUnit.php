...
class ProfitUnit extends ApiController
{
	...
	public function insert()
	{
		if($post = $this->input->post()){
            ...
            $data['profit_unit']     = $this->input->post('profit');	
            // $data['amount_outstanding'] = $this->input->post('outstanding');
            if($find = $this->kpi->find(array(
                'year'  => $this->input->post('year') ,
                "month" =>  $this->input->post('month') + 1 ,
                "id_unit"=> $this->input->post('unit')
            ))){

                // print_r( $data['profit_unit'] ); exit;
                $this->db->set('profit_unit', $data['profit_unit']);
                $this->db->where('id', $find->id);            
                $data =  $this->kpi->db->update('units_kpi' );

                 return $this->sendMessage(true,'Update Berhasil',200);

                // $this->db->set('profit_unit', $this->input->post('profit_unit'));
                // $this->kpi->update($data, $find->id);
