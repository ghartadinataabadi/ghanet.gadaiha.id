...
class Hps extends Authenticated
{
	...
	public function index()
	{
		...
		$region = [];
		$page = $this->syariah->region(1)->total_page;
		for($a=1; $a<=$page; $a++){
				$region[$a] = $this->syariah->region($a)->data;
		}

		$insurance_item = $this->syariah->insurance_item()->data->insurance_item_product->insurance_items;
	
		// $merk_hps = $this->gcore->merk();
    // $merk = $this->gcore->merk_hps()->data;


		$this->load->view('syariah/hps/index', array(
			'areas'	=> $this->areas->all(),
      'region' => $region,
			// 'merk' => $merk,
			'insurance_item' => $insurance_item,
			'url' => $this->transaction_syariah,
		));
	}
