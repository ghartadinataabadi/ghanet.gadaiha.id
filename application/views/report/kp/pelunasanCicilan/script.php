...
function initCariForm(){
    ...
    $('#btncari').on('click',function(){
		...
		$.ajax({
			type : 'GET',
			url : "<?php echo base_url("api/gcore/pelunasanCicilan"); ?>",
			dataType : "json",
			...
			success : function(response,status){
				...
				if(response.status == true){
					...
					$.each(response.data, function (index, data) {
                        ...
                        template += "<td class='text-right'>"+data.tenor+"</td>";
						template += "<td class='text-center'>"+convertToRupiah(data.monthly_fee * data.tenor)+"</td>";                        
						template += "<td class='text-right'>"+ fine_amount +"</td>";
