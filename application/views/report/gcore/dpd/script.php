...
function initCariForm() {
    ...
    $('#btncari').on('click', function() {
        ...
        $.ajax({
            type: 'GET',
            url: "<?php echo base_url("api/gcore/kp/dpd"); ?>",
            dataType: "json",
            data: {
                area_id: area_id,
                date: date,
            ...
            success: function(response, status) {
                ...
                if (response.status == true) {
                    ...
                    $.each(response.data, function(index, data) {
                        ...
                        if(data.product_name == 'Gadai Reguler' || data.product_name == 'Gadai Reguler GHTS'){
                            var date_dpd = moment(data.due_date).add(8, 'days');
                            dpd = parseInt(date_between(`${data.due_date}`,
                            "<?php echo date('Y/m/d');?>")) - 7;
                            
                        }else{
                            var date_dpd = moment(data.due_date).add(1, 'days');
                            dpd = parseInt(date_between(`${data.due_date}`,
                            "<?php echo date('Y/m/d');?>"));
                        }
                        ...
                        template += "<td class='text-center'>" + moment(data.due_date)
                            .format('DD-MM-YYYY') + "</td>";
                        template += "<td class='text-center'>" + moment(date_dpd)
                            .format('DD-MM-YYYY') + "</td>";
                        ...
                        }else if(data.product_name == 'Gadai Opsi ACC'){
                            sewa = Math.ceil(data.tafsiran_sewa / 4);
                            template += "<td class='text-right'>" + convertToRupiah(sewa) + "</td>";
                        ...
                        }else if(data.product_name == 'Gadai Elektronik'){
                        
                            sewa = Math.ceil(data.tafsiran_sewa);
                            template += "<td class='text-right'>" + convertToRupiah(sewa) + "</td>";
                            template += "<td class='text-right'>" + convertToRupiah(
                                calculateDendaElektronik(data.loan_amount, dpd, data.interest_rate)) + "</td>";
                            var up = parseInt(calculateDendaElektronik(data.loan_amount, dpd, data.interest_rate));
                            var calcup = up + parseInt(sewa) + parseInt(data.loan_amount);
														totalDenda += calculateDendaElektronik(data.loan_amount, dpd, data.interest_rate);
                        }else{
...
function calculateDendaElektronik(up, dpd, rate) {
    var sumDay = dpd;

    if(sumDay > 0){
      var calculate = sumDay * up * (rate/2) / 100 / 15;
			var data = Math.ceil(parseInt(calculate));
			return data;
    }
    return 0;
}
