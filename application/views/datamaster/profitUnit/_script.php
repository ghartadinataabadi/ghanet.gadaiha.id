...
var monthName = ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September',
              'Oktober',
              'November', 'Desember'
          ];
...
function initCariForm(){
    ...
    $('#btncari').on('click',function(){
		...
		$.ajax({
			type : 'GET',
			url : "<?php echo base_url("api/datamaster/ProfitUnit"); ?>",
			dataType : "json",
			...
			success : function(response,status){
                ...
                filterUnit.forEach((unit, index)=>{
                    ...
                    filterMonth.forEach(month=>{
                        const getTarget = targetUnits.filter(data=>data.id_unit == id && data.year == getYear && data.month-1 == month.id);
                        // let amount_outstanding = 0;
                        let profit_unit = 0;
                        if(getTarget.length > 0){
                            profit_unit = getTarget[0].profit_unit;
                        ...
                        html += `<td><input value="${profit_unit}" id="profit_unit" name="profit_unit" onkeyup="moneyHander(event)" class="form-control money_format" type="text"  ></td>`;
                        ...
                        // var profit_unit = $("input[name='profit_unit']").val();
                        // profit_unit.value = convertToRupiah(this.value);
    ...
    // var profit_unit = tr.querySelector('[name="profit_unit"]').value;
    // var amount_outstanding = tr.querySelector('[name="amount_outstanding"]').value;

                    // var profit_unit = document.querySelector('.money_format');
                    // profit_unit.addEventListener('keyup', function(e)
                    // {
                    //     profit_unit.value = formatRupiah(profit_unit.value);
                    // });
                    ...
                    // });
                        ...
                        // var profit_unit = document.getElementById('profit_unit');
                        // var amount_outstsanding = document.getElementById('amount_outstsanding');

                        // profit_unit.addEventListener('keyup', function(e){convertNumber(); hitung();});
                        ...
                        // function hitung(){
                        //     var profit_unit   = $("#profit_unit").val();
                    ...
                    // var booking      = document.getElementById('profit_unit');
...
$(document).on('click','.btn-save', (element)=>{
    ...
    // const book   = tr.querySelector('[name="profit_unit"]').value;
    // const outs  = tr.querySelector('[name="amount_outstanding"]').value;
    // const profit_unit = formatNumber(book)) ;
    // const amount_outstanding = formatNumber(outs));

    const tr = element.target.closest('td').closest('tr');
    // const getamount_booking = tr.querySelector('[name="profit_unit"]').value;
        // const getamount_booking = document.getElementById('profit_unit').value;
    ...
    const profit_unit = formatNumber(tr.querySelector('[name="profit_unit"]').value);
    ...
    $.ajax({
        type : 'POST',
        url : "<?php echo base_url('api/datamaster/ProfitUnit/insert');?>/",
        dataType : "json",
        data:data,
        success : function(response,status){
            KTApp.unblockPage();
