<script>
	...
	function initDTEvents() {
		$(".btn_delete").on("click", function() {
			...
			swal.fire({
				...
				if (result.value) {
					...
					$.ajax({
						type: 'GET',
						url: "<?php echo base_url("api/syariah/hps/delete"); ?>",
						data: {
							id: targetId
						},
						dataType: "json",
		...
		$(".btn_edit").on("click", function() {
			...
			$.ajax({
				type: 'GET',
				url: "<?php echo base_url("api/syariah/hps/show"); ?>",
				data: {
					id: targetId
				},
				dataType: "json",
    ...
    $("#btn_edit_submit").on("click", function() {
        ...
        if (isValid) {
            ...
            $.ajax({
                type: 'POST',
                url: "<?php echo base_url("api/syariah/hps/update"); ?>",
    ...
    //$('#add_cabang').val("val", "").trigger('change.select2');
		...
		$("#btn_add_submit").on("click", function() {
			...
			if (isValid) {
				...
				$.ajax({
					type: 'POST',
					url: "<?php echo base_url("api/syariah/hps/insert"); ?>",
	...
	$('.form-input').on('submit', function (e) {
		...
		console.log(data);
		$.ajax({
			url : '<?php echo base_url('api/syariah/hps/upload');?>',
			type : 'POST',
			data : data,
			async: true,
			cache: false,
	...
	function initDataTable(){		
		...
		var option = {
			data: {
				...
				source: {
					read: {
						url: `<?php echo base_url("api/syariah/hps"); ?>?area=${area}&merk=${merk}&insurance=${insurance}&processor=${processor}`,
						map: function(raw) {
							// sample data mapping
							var dataSet = raw;
	...
	$('#merk').on('change', function() {
		...
		$('#insurance').on('change', function() {
			...
			$.ajax({
					type: 'GET',
					url: "<?php echo base_url("api/syariah/hps/merk_hps"); ?>/" + insurance,
					dataType: "json",
